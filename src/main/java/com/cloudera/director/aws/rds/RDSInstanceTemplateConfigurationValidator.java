...
public class RDSInstanceTemplateConfigurationValidator implements ConfigurationValidator {
  ...
  static final String INVALID_ENGINE = "Invalid engine (not one of " +
      RDSEngines.ENGINES.keySet() + ") : %s";
  ...
  void checkEngine(AmazonRDSClient client,
    ...
    // However, in the unlikely event that something goes wrong, use the RDS-specific
    // shadowing ENGINE property in preference to the generic TYPE.
    String type = configuration.getConfigurationValue(ENGINE, localizationContext);
    ...
    try {
      ...
      addError(accumulator, ENGINE, localizationContext,
    ...
    String engine;
    try {
      engine = RDSEngines.getEngine(databaseType);
    } catch (IllegalArgumentException e) {
      addError(accumulator, ENGINE, localizationContext,
          null, e.getMessage());
      return;
