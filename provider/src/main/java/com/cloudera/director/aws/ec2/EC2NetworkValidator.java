...
import static com.cloudera.director.aws.ec2.EC2InstanceTemplateConfigurationValidator.INVALID_SECURITY_GROUP;
    ...
    for (SecurityGroup sg : securityGroups) {
      ...
      for (IpPermission permission : permissions) {
        ...
        if (!ipRanges.isEmpty()) {
          ...
          Iterables.removeIf(notEnforcedRules, new Predicate<NetworkRule>() {
            @Override
            public boolean apply(NetworkRule rule) {
              return rule.isEnforced(protocol, ports, ipRanges, AccessType.ALLOW);
            }
          });

          // Check violations for deny rules
          violatedRules.putAll(sg.getGroupId(), Iterables.filter(denyRules, new Predicate<NetworkRule>() {
            @Override
            public boolean apply(NetworkRule rule) {
              return rule.isViolated(protocol, ports, ipRanges, AccessType.ALLOW);
            }
          }));
        }
      }
