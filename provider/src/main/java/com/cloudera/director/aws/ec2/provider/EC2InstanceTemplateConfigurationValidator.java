 ...
 */
@SuppressWarnings({"PMD.TooManyStaticImports", "PMD.UnusedPrivateField", "PMD.UselessParentheses",
    "unused"})
public class EC2InstanceTemplateConfigurationValidator implements ConfigurationValidator {
  ...
  static final String INVALID_AMI_OWNER_MSG =
      "Only certain Linux platforms are supported. "
          ...
          + "the Cloudera Altus Director User Guide. "
  ...
  @VisibleForTesting
  static final String INVALID_SPOT_PRICE_MSG =
      "Invalid Spot price %s. Spot price must be a positive value representing the price in USD/hr";
  ...
  @VisibleForTesting
  static final String NO_SPOT_WITH_ASG_MSG =
      "Spot Instance use with Auto Scaling Groups is not currently supported";
    ...
    for (String securityGroupId : securityGroupsIds) {
      ...
      try {
        ...
        if (result.getSecurityGroups().size() == 1) {
          ...
          Set<String> sgSet = vpcSgMap.computeIfAbsent(vpcId, k -> Sets.newHashSet());
  ...
  void checkEbsVolumes(AWSKMSClient kmsClient, Configured configuration,
    ...
    if (ebsVolumeCount > 0) {
      ...
      checkEbsIops(accumulator, localizationContext, configuration, metadata);
    }
  }

  private void checkEbsIops(PluginExceptionConditionAccumulator accumulator,
  ...
  void checkSpotParameters(Configured configuration,
    ...
    boolean useSpotInstances = Boolean.parseBoolean(
        configuration.getConfigurationValue(USE_SPOT_INSTANCES, localizationContext));
    String spotPriceUSDPerHr =
        configuration.getConfigurationValue(SPOT_PRICE_USD_PER_HR, localizationContext);
    if (spotPriceUSDPerHr == null || spotPriceUSDPerHr.isEmpty()) {
        spotPriceUSDPerHr = configuration.getConfigurationValue(SPOT_BID_USD_PER_HR, localizationContext);
    }
    ...
    boolean automatic = Boolean.parseBoolean(
        configuration.getConfigurationValue(AUTOMATIC, localizationContext));
    if (automatic && useSpotInstances) {
      addError(accumulator, USE_SPOT_INSTANCES, localizationContext,
          null, NO_SPOT_WITH_ASG_MSG);
    }

    if (spotPriceUSDPerHr != null && !spotPriceUSDPerHr.isEmpty()) {
      boolean valid = false;
      try {
        BigDecimal spotPrice = new BigDecimal(spotPriceUSDPerHr);
        valid = spotPrice.compareTo(BigDecimal.ZERO) > 0;
      } catch (NumberFormatException ignore) {
      }
      if (!valid) {
        addError(accumulator, SPOT_PRICE_USD_PER_HR, localizationContext,
            null, INVALID_SPOT_PRICE_MSG, spotPriceUSDPerHr);
      }
    }
