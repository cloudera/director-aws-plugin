...
/**
 * <p>Tests EC2Provider.</p>
 * <p>
 * <p><em>Note:</em> This test is deliberately placed in the provider module, rather than the tests
 * module, due to the inconveniences introduced by shading. </p>
 */
public class EC2ProviderLocalTest {
  ...
  private static final EC2InstanceTemplate TEST_TEMPLATE = new EC2InstanceTemplate("instance-template",
      new SimpleConfiguration(TEMPLATE_CONFIGS), ImmutableMap.of(), LOCALIZATION_CONTEXT);
  ...
  public void testAllocate() throws ExecutionException, InterruptedException {
    ...
    when(ec2Client.runInstancesAsync(any(RunInstancesRequest.class))).then(
        (Answer<Future<RunInstancesResult>>) invocation -> {
  ...
  public void testAllocateSomeTerminateWhileWaitingForIp() throws ExecutionException, InterruptedException {
    ...
    when(ec2Client.describeInstances(any(DescribeInstancesRequest.class)))
        .thenAnswer((Answer<DescribeInstancesResult>) invocation -> {
  ...
  private EC2Provider getTestEc2Provider() {
    return getTestEc2Provider(ImmutableMap.of());
  }
  ...
  private EC2Provider getTestEc2Provider(Map<String, String> tagMappings) {
    ...
    VirtualizationMappings virtualizationMappings = new VirtualizationMappings(new SimpleConfiguration(),
        new File("dummy"), LOCALIZATION_CONTEXT);
    AWSFilters awsFilters = new AWSFilters(Optional.absent());
    ...
    when(autoScalingClientProvider.getClient(any(Configured.class), any(PluginExceptionConditionAccumulator.class),
        any(LocalizationContext.class), anyBoolean())).thenReturn(autoScalingClient);
    ClientProvider<AmazonIdentityManagementClient> identityClientProvider = mock(ClientProvider.class);
    AmazonIdentityManagementClient identityClient = mock(AmazonIdentityManagementClient.class);
    ...
    ClientProvider<AWSKMSClient> kmsClientProvider = mock(ClientProvider.class);
